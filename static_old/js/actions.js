...
$(document).ready(function () {
	...
	$('.check_items_row input.check_item').prop('checked', false).change();
	...
	$('a.check_all').click(function (e) {
		...
		if (checked) {
			...
			$('.check_items_row input.check_item').prop('checked', true).change();
		}
	...
	$(document).on('submit', 'form.ajax', function (e) {
		$(this).find('button, input[type="submit"]').attr('disabled', '');
		if ($(this).hasClass('submitting')){
			var submitting_ok = false;
		}else{
			$(this).addClass('submitting');
			var submitting_ok = true;
		}
				...
				jQuery.ajax({
					...
					success: function (data) {
						...
						if (status == 'true') {
							...
							// swal({ title: title, text: message, type: "success" }, setTimeout(doAfter() ), '9000');
							if (isCloseModal){
